...
export class AssessorComponent {
  ...
  insertDataIntoPDF = async (assessment: any, download: boolean) => {
    try {
      if (assessment.ongoing == undefined) {
        this.openSnackBar("Report is not filled yet", "Close")
        return
      }
      ...
      if (localStorage.getItem("pdfData")) {
        ...
        this.openSnackBar("There is no Empty Report", "Close");
      }
      if (pdfDoc) {
        //helper functions
  ...
  fetchSchoolsAndStudents() {
    try {
      ...
      console.log(error);
      this.openSnackBar("Unable to fetch schools and pdf", "Close")
    }
  }
